# raycast-collection

A personal collection of [Raycast](https://www.raycast.com/?via=73820f).

## Development

Official

- [Raycast Store](https://www.raycast.com/store?via=73820f)
- [Raycast Developer Doc](https://developers.raycast.com/?via=73820f)
- [Raycast Extensions Repository](https://github.com/raycast/extensions)

Personal

- [如何开发一个 Raycast 扩展？](https://github.com/tofrankie/blog/issues/364)
- [当 Raycast extensions 仓库过大，如何更方便提交 PR？](https://github.com/tofrankie/blog/issues/391)

## Authored Extensions

[Lists](https://www.raycast.com/tofrankie?via=73820f)

| Extension                 | Description                                                | Install                                                                                          |
| :------------------------ | :--------------------------------------------------------- | :----------------------------------------------------------------------------------------------- |
| **WeChat DevTool**        | 用于快速启动微信小程序项目                                 | [Install from Raycast Store](https://www.raycast.com/tofrankie/wechat-devtool?via=73820f)        |
| **Atlassian Data Center** | 适用于私有部署的 Confluence、Jira， 可快速检索文章和 Issue | [Install from Raycast Store](https://www.raycast.com/tofrankie/atlassian-data-center?via=73820f) |
| **Chinese Converter**     | 数字转大写金额                                             | [Install from Raycast Store](https://www.raycast.com/tofrankie/chinese-converter?via=73820f)     |

## Contributed Extensions

[Lists](https://www.raycast.com/tofrankie?via=73820f)

## License

MIT License © [Frankie](https://github.com/tofrankie)
